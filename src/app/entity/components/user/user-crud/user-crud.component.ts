...
export class UserCrudComponent implements OnInit {
  ...
  create() {
    ...
    this.visibleControls.hash = false;
    this.form.reset();
    this.hide = true;    
    this.user = null;
  }
      ...
      this.entityService.request('post', UserModel.entity, this.user)
        .subscribe(res => {
          ...
          if (res.ok) {
            ...
            let succesMessage = "Nuevo usuario: " + this.user.name;
      ...
      this.entityService.request('put', UserModel.entity, this.user)
        .subscribe(res => {
          ...
          if (res.ok) {
            //Success
            this.user = <UserModel>res.user;
            let succesMessage = "Actualización de usuario: " + this.user.name;
            this.openSnackBar(succesMessage, "X", "snackbar-success");
            ...
            let errorMessage = "¡Error, " + res.menssage + "!";
            this.openSnackBar(errorMessage, "X", "snackbar-danger");
            this.eventUpdateListEmitter(true); //Para que limpie el objeto en la lista
          }
        });
  ...
  onDelete() {
    ...
    //Succes
    let succesMessage = "Se eliminó el usuario: " + this.user.name;
    this.openSnackBar(succesMessage, "X", "snackbar-success");
  }
  ...
  getErrorMessageName() {
    ...
    if (this.form.get('name').hasError('minlength')) {
      return `El nombre debe de contener al menos ${this.form.get('name').errors.minlength.requiredLength} caracteres`;
    }
  }
  ...
  getErrorMessagePassword() {
    ...
    if (this.form.get('password').hasError('minlength')) {
      return `La contraseña debe de contener al menos ${this.form.get('password').errors.minlength.requiredLength} caracteres`;
    }
  }
